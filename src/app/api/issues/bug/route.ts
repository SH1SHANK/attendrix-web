...
const bodySchema = z.object({
  ...
  steps: z.string().max(3000).optional(),
  expected: z.string().max(3000).optional(),
  actual: z.string().max(3000).optional(),
...
export async function POST(request: Request) {
  try {
    ...
    const data = {
      ...
      steps: getString(formData.get("steps")),
      expected: getString(formData.get("expected")),
      actual: getString(formData.get("actual")),
    ...
    const bodyText = [
      ...
      "",
      "## Steps to Reproduce",
      parsedData.steps || "Not provided",
      "",
      "## Expected Behavior",
      parsedData.expected || "Not provided",
      "",
      "## Actual Behavior",
      parsedData.actual || "Not provided",
      "",
      "## Affected Page",
